...
interface RateExternalRestaurantModalProps {
  open: boolean;
...
const RateExternalRestaurantModal = ({
  open,
  onOpenChange,
  offerId,
  ...
  motoboyId,
  onRatingComplete,
}: RateExternalRestaurantModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error } = await supabase.from("external_restaurant_ratings").insert({
        offer_id: offerId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md overflow-hidden">
        ...
        <div className="py-6">
          ...
          >
            Esta avaliação será visível para outros motoboys
          </motion.p>
        </div>
...
export default RateExternalRestaurantModal;
