...
interface RateMotoboyModalProps {
  open: boolean;
    ...
    try {
      ...
      setTimeout(() => {
        toast({
          ...
          description: `Obrigado por avaliar ${motoboyName}.`,
        });
        onRatingComplete();
      }, 150);
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-md overflow-hidden">
        <DialogHeader className="text-center pb-2">
          ...
          <DialogDescription className="text-center">
            ...
            <span className="font-semibold text-primary">{motoboyName}</span>?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-6">
          ...
          <motion.div className="mt-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            <RatingTagSelector applicableTo="motoboy" selectedTags={selectedTags} onTagsChange={setSelectedTags} />
          </motion.div>
          ...
          <motion.div className="mt-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <Textarea
              ...
              maxLength={500}
            />
            <p className="text-xs text-muted-foreground mt-2 text-right">{comment.length}/500 caracteres</p>
          </motion.div>
          ...
          >
            Esta avaliação será visível para outros restaurantes e motoboys ofertantes
          </motion.p>
        </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button onClick={handleSubmit} disabled={rating === 0 || isSubmitting} className="gap-2 min-w-[140px]">
            {isSubmitting ? (
...
export default RateMotoboyModal;
